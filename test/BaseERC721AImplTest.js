...
describe('BaseERC721AImpl', () => {
    ...
    describe('only owner', () => {
        ...
        it('should cannot be called by non-owner - setPrice', async () => {
            await expect(
                nft.connect(wallet1).setPrice(price)
        ...
        it('should cannot be called by owner - setBaseURI', async () => {
            await expect(
                nft.connect(wallet1).setBaseURI(baseURI)
            ).to.be.revertedWith('Ownable: caller is not the owner');
        });
        ...
        it('should cannot be called by owner - setWallet', async () => {
            await expect(
                nft.connect(wallet1).setWallet(_wallet)
            ).to.be.revertedWith('Ownable: caller is not the owner');
        });
        ...
        it('should cannot be called by owner - setSigner', async () => {
            await expect(
                nft.connect(wallet1).setSignerWallet(_signer)
            ).to.be.revertedWith('Ownable: caller is not the owner');
        });
        ...
        it('should cannot called by owner - airdrop', async () => {
            await expect(
                nft.connect(wallet1).airdrop([wallet1.address], 1)
            ).to.be.revertedWith('Ownable: caller is not the owner');
        });
    ...
    describe('update contract', async () => {
        it('should can update price', async () => {
            ...
            await nft.connect(owner).setPrice(parseEther('0.2'));
            const after = await nft.price();
            expect(before).to.not.eq(after);
            expect(after).to.eq(parseEther('0.2'));
        });
    ...
    describe('airdrop', async () => {
        it('should can airdrop to the wallets', async () => {
            await nft
                .connect(owner)
                .airdrop([wallet1.address, wallet2.address], 2);
        ...
        it('should can airdrop dynamically to the wallets', async () => {
            await nft
                .connect(owner)
                .airdropDynamic([wallet3.address, wallet4.address], [4, 8]);
    ...
    describe('mint', async () => {
        ...
        it('should cannot mint nfts if not sufficient eth', async () => {
            await nft.connect(owner).setPrice(parseEther('0.1'));
            await expect(
                nft.connect(wallet2).mint(5, { value: parseEther('0') })
    ...
    describe('signature', async () => {
        it('should can pass with correct signature and nonce', async () => {
            await nft.setSignerWallet(wallet.address);
            const sig = await getWhitelistSignature(wallet1.address, wallet);
            const check = await nft
                .connect(wallet1)
                .testSign(sig.nonce, sig.sig);
            expect(check).to.eq(true);
        });

        it('should cannot pass with incorrect signature and nonce', async () => {
            await nft.setSignerWallet(wallet1.address);
            const sig = await getWhitelistSignature(wallet2.address, wallet);
            const check = await nft
                .connect(wallet2)
                .testSign(sig.nonce, sig.sig);
